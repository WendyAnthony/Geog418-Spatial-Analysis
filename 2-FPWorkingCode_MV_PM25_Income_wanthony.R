...
plot(pm.income.poly$PM25.mean~pm.income.poly$Income, main = "Plot of PM2.5 Mean ~ Income")
png("pm.income.poly_PM25.mean_Income.png")
plot(pm.income.poly$PM25.mean~pm.income.poly$Income, main = "Plot of PM2.5 Mean ~ Income")
dev.off()

## ----Regression_model_2---------------------------------------------
lm.model.2 <- lm(pm.income.poly$PM25.mean~pm.income.poly$Income)

## -------------------------------------------------------------------
summary(lm.model.2)

## ----Plot_of_income_PM25_abline-------------------------------------
plot(pm.income.poly$PM25.mean~pm.income.poly$Income, main = "Plot of PM2.5 ~ Income")
abline(lm.model.2, col="red", lw=2,lty=2)

png("pm.income.poly_PM25_Income.mean_2.png")
plot(pm.income.poly$PM25.mean~pm.income.poly$Income, main = "Plot of PM2.5 Mean ~ Income")
abline(lm.model.2, col="red", lw=2, lty=2)
dev.off()

## ----Get_residuals_from_model2--------------------------------------
model.2.resids <- as.data.frame(residuals.lm(lm.model.2))
...
## ----Map_residuals2_PM25_data_with_sample---------------------------
map_pm25_vit.data_regress_residuals2 <- tm_shape(pm.income.poly) + 
  tm_polygons(col = "residuals2", 
              title = "Regression residuals2", 
              style = "fisher", 
              palette = "viridis", n = 6,
              border.col = "grey", 
              border.alpha = 0.05) +
  tm_legend(legend.outside=FALSE, position = c(0.01, 0.01)) + 
  tm_layout(inner.margins = c(.12, .17, .15, .03), title = "Metro Vancouver 2016 Census Tracts\nPM2.5 ~ Income Regression residuals2", title.position = c("LEFT", "TOP")) + 
  tm_scale_bar(position = c(0.4, 0.05)) +  
  tm_compass(type= "4star", position=c("RIGHT", "TOP")) 
# Use "fisher" instead of "jenks" for larger data sets
...
vit.lw <- nb2listw(vit.nb, zero.policy = TRUE, style = "W")
print.listw(vit.lw, zero.policy = TRUE)
AutoCorr_afterReg_print.listw_2 <- print.listw(vit.lw, zero.policy = TRUE)

## -------------------------------------------------------------------
pm.income.poly$IncLagMeans.res_2 = lag.listw(vit.lw, pm.income.poly$residuals2, zero.policy = TRUE)
head(pm.income.poly)

AfterReg.IncLagMeans_2 <- pm.income.poly$IncLagMeans.res_2

## -------------------------------------------------------------------
map_LagMean_res_2 <- tm_shape(pm.income.poly) + 
  tm_polygons(col = "IncLagMeans.res_2", 
              title = "Median 2016 $ Income\nLagged Means", 
              style = "fisher", 
              palette = "viridis", n = 6,
              border.col = "grey", 
...
## -------------------------------------------------------------------
mi <- moran.test(pm.income.poly$residuals2, vit.lw, zero.policy = TRUE)
mi
...
moran.range(vit.lw)  
moran.range.afterReg_2 <- moran.range(vit.lw)
...
## -------------------------------------------------------------------
data.for.table1 = data.frame(mI, eI, var, z)
write.csv(data.for.table1, "moransGlobal_vit_Income_afterRegression_2.csv", row.names = FALSE)

## ----Lisa_Test_afterRegression_2--------------------------------------
lisa.test <- localmoran(pm.income.poly$residuals2, vit.lw)
...
## -------------------------------------------------------------------
map_LISA_res_2 <- tm_shape(pm.income.poly) + 
  tm_polygons(col = "Ii", 
              title = "\nLocal Moran's i", 
              style = "fisher", 
              palette = "viridis", n = 6,
...
## -------------------------------------------------------------------
# > use manual breaks
map_LISA_Z_res_2 <- tm_shape(pm.income.poly) + 
  tm_polygons(col = "Z.Ii", 
              title = "\nLocal Moran's i Z values", 
              breaks = c(-Inf, -2.326, -1.96, 1.96, 2.326, Inf ),
              palette = "PuOr", n = 6,
...
## -------------------------------------------------------------------
map_LISA_P_res_2 <- tm_shape(pm.income.poly) + 
  tm_polygons(col = "P", 
              title = "\nLocal Moran's i P values",
              breaks = c(0, 0.001, 0.01, 0.05, Inf ),  # 95% p=0.05
              palette = "-BuPu", n = 5,
...
## -------------------------------------------------------------------
moran.plot(pm.income.poly$residuals2, vit.lw, zero.policy=NULL, spChk=NULL, labels=NULL, xlab="Pm2.5 Mean ~ Income Residuals_2", ylab="Spatially Lagged Pm2.5 Mean ~ Income Residuals_2", quiet=NULL, main ="Local Moran's i Residuals Plot \nMetro Vancouver 2016 Pm2.5 Mean ~ Income (after Regression_2)")

png("moran.plot_vit_Residuals_afterRegression_2.png")  
moran.plot(pm.income.poly$residuals2, vit.lw, zero.policy=NULL, spChk=NULL, labels=NULL, xlab="PM2.5 ~ Income Residuals", ylab="Spatially Lagged Pm2.5 Mean ~ Income Residuals", quiet=NULL, main ="Local Moran's i Residuals Plot \nMetro Vancouver 2016 PM2.5 ~ Income (after Regression_2)")
dev.off()
...
GWRbandwidth_2 <- gwr.sel(pm.income.poly$PM25.mean~pm.income.poly$Income, 
...
gwr.model_2 = gwr(pm.income.poly$PM25.mean~pm.income.poly$Income, 
                data=pm.income.poly, coords=cbind(pm.income.poly$X, pm.income.poly$Y), 
                adapt=GWRbandwidth_2, hatmatrix=TRUE, se.fit=TRUE) 
#Print the results of the model
gwr.model_2

## ----GWR_results_SDF------------------------------------------------
#Look at the results in detail
results_2 <- as.data.frame(gwr.model_2$SDF)
head(results_2)

## ----Add_local_r_square_values_to_map-------------------------------
pm.income.poly$localr_2 <- results_2$localR2
pm.income.poly$localr_2
head(pm.income.poly)

## ----Create_choropleth_map_of_r_square_values-----------------------
local.r.square_2 <- pm.income.poly$localr_2
...
## ----Map_coefficient------------------------------------------------
map_gwr_coefficient_2 <- tm_shape(pm.income.poly) + 
